...
DB_URL="postgresql+asyncpg://moviepicker:${DB_PASSWORD}@postgresql.${NAMESPACE}.svc.cluster.local:5432/moviepicker?ssl=require"
...
kubectl --context "$CONTEXT" -n "$NAMESPACE" create secret generic movie-picker-api-secrets \
  ...
  --dry-run=client -o yaml | apply_or_seal "movie-picker-api-secrets"
